...
node stream-consumer.js  # Kafka consumer, counts what it sees
...
| file | |
|---|---|
| `model/place-order-handler.js` | the handler. **Nothing in it is wrong** |
| `simple-eventing/event-stream-consumer.js` | the stream's version of the pump. Read the comment at the top |
| `simple-eventing/stream.js` | the topology — and compare its length with `simple-messaging/channel.js` |

1. **`channel.js` declares four queues. `stream.js` declares one topic.** Exercise 2 used the
...
```
DUAL_WRITE_WINDOW=15 node receiver.js
```
...
**Now do it the other way round.** Move the acknowledgement in `simple-messaging/message-pump.js`
so it happens *before* `this.#handler.handle(...)` instead of after. Reset, and run the probe again — but
...
```
node sender.js slow  # then kill the receiver during the lookup, before the event
```
...
```
DUAL_WRITE_WINDOW=15 node receiver.js  # terminal 1
node sender.js                         # terminal 2, then look at ../00-setup/queues.sh
```
...
**Put the acknowledgement back after `this.#handler.handle(...)` before you go on.** Probes B and C do
...
```
../00-setup/reset.sh
node stream-consumer.js   # terminal 1, leave it running
node sender.js bad-event  # appends an unreadable record to the topic
```
...
```
node receiver.js        # terminal 2
node sender.js burst 6  # terminal 3
../00-setup/lag.sh
```
...
> probe's answer.** The poison record is keyed with a fixed string by `sender.js`, so it
> lands on the same partition every time you run this. Each good order's event is keyed by the
> order id, and the publisher is wired up in `receiver.js` — so those scatter across all
...
```
../00-setup/reset.sh
node receiver.js        # terminal 1, then send a few good orders and stop it
node sender.js burst 4  # terminal 2
```

Now change `CONSUMER_GROUP` in `simple-eventing/stream.js` to any new name and run the stream
consumer again.
