...
export function retryQueueNameFor(messageClass) {
  return 'retry.' + queueNameFor(messageClass);
}

/** Bodies we could not read. Terminal: nothing here is ever retried. */
export function invalidQueueNameFor(messageClass) {
  return 'invalid.' + queueNameFor(messageClass);
}

/** Work we retried and gave up on. Terminal: an operator's in-tray. */
export function deadLetterQueueNameFor(messageClass) {
  return 'dead.' + queueNameFor(messageClass);
}
