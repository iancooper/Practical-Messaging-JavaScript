...
`Catalogue` asks `PriceStore` for a price. `PriceStore` is declared by
`model`, implemented by `local-copy/`, and wired together in `receiver.js`.
...
order and never will. **Your `PriceStore` interface is where you decide whether that is
...
**It told you.** `PriceStore` can date the copy, the receiver prints it, and that is
