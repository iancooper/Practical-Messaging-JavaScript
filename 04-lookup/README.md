...
`model/catalogue.js` is a `Map<string, number>`. It has been standing in for reference
...
1. **A `PriceChanged` event and a topic for it.** A SKU and a price, on the Kafka stream
   gateway in `simple-eventing/` that exercise 3 gave you, keyed so that events for
...
4. **`Catalogue` reads the local copy** instead of its dictionary. `priceOf` no longer throws
   `CatalogueUnavailableError`, because there is nothing left to be unavailable.

**SQLite is the cheapest durable store there is**, and in Node it is cheaper than that:
`node:sqlite` is built in. No package, no native addon, nothing to install, and the
database is a file you can delete. It does print `ExperimentalWarning: SQLite is an
experimental feature` on every run -- that is Node saying the API may still change, not
...
declares** — "given a SKU, what is the price, and do you have one at all?" `local-copy/`
implements it, and is the only place that names SQLite. `receiver.js` puts the two together,
...
**And it is a check rather than a matter of opinion.** `check-domain.js` already
walks `model/` and fails if anything under it names the broker's library. **Add
`node:sqlite` to the list it rejects.** JavaScript has no compiler to do this for you, and
a rule nothing enforces is a rule you will break in the second hour of a take-home.

### The `PlaceOrderHandler` must not change either ###

If it does, the lookup has leaked into your domain the way `GetMessage` did in exercise 1 —
...
```
node price-consumer.js     # terminal 1, leave it running
node receiver.js           # terminal 2, leave it running
node price-seeder.js seed  # terminal 3: publish a starting price for every SKU
node sender.js             # terminal 3: place an order
```
...
```
node price-seeder.js set WIDGET-1 11.99  # terminal 3, watch terminal 1 print the gap
node sender.js                           # then place an order and see which price it got
```
...
node price-seeder.js set WIDGET-1 49.99
node sender.js
```
...
```
node sender.js         # WIDGET-1: fine, but we have no copy yet
# ...now start the price consumer and seed it, then...
node sender.js poison  # NOPE-404: not a thing, and never will be
```
...
```
PRICE_WRITE_WINDOW=15 node price-consumer.js  # terminal 1
node price-seeder.js set WIDGET-1 77.77       # terminal 3
kill -9 <the price consumer's PID>            # while the window is open
node price-consumer.js                        # start it again and watch
```
...
**Now reverse the two lines in `price-consumer.js`** — commit the offset first, then apply —
and run it again.
