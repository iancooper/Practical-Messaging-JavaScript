...
The pump had one `catch (e)` and one response to it: `requeue`, which is
...
**One `catch`, two answers, because there are two failures.** The `instanceof` test is doing
the work a typed `catch` clause does in the other languages; what matters is the distinction,
not the mechanism that expresses it.

### A failure to understand: `UnmappableMessageError` ###
