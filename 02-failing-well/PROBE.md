...
- `PlaceOrderHandler.handle` takes a `PlaceOrder`. `check-domain.js` has no broker reference
...
```
cd 02-failing-well
node receiver.js  # terminal 1
node sender.js    # terminal 2
```
...
|---|---|
| `simple-messaging/channel.js` | the topology, drawn in the comment. Read the comment |
| `simple-messaging/data-type-channel-consumer.js` | the five things the pump can do with a message |
| `simple-messaging/message-pump.js` | the `catch` block. All of it |
...
3. **The `catch` block catches `everything`.** Name two failures that both land in it and should
...
```
node receiver.js | tee /tmp/probe-a.log  # terminal 1
node sender.js unmappable                # terminal 2
```
...
```
node sender.js poison  # NOPE-404: never in the catalogue, ever
```
...
```
../00-setup/peek.sh dead.failing-well.PlaceOrder
```
...
**One file: `simple-messaging/message-pump.js`.** The topology is already declared and correct;
...
**Sort the one `catch` into two paths, because you have two kinds of failure.** JavaScript has
no typed `catch` clause — one `catch (e)` takes whatever was thrown — so the two paths are an
`instanceof` test inside the block you already have:

**A failure to *understand*** — the mapper threw `UnmappableMessageError`. The bytes are
...
|---|---|---|
| `unmappable` | 1 | `invalid.failing-well.PlaceOrder` |
| `poison` | 4 | `dead.failing-well.PlaceOrder` |
| `flaky` | 3 | placed successfully, nowhere |
| nothing | — | `retry.…` **empty**, and it should be empty nearly all the time |

And with all of that going on, `node sender.js` must still place a good order
