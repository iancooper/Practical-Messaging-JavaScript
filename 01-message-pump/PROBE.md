...
```
cd 01-message-pump
node receiver.js  # terminal 1: the pump
node sender.js    # terminal 2: sends one order
```
...
Find *Queues* → `message-pump.PlaceOrder`. Keep it open. Your agent cannot see it and cannot read it —
...
| file | |
|---|---|
| `simple-messaging/message-pump.js` | the pump: **Get → Translate → Dispatch → Handle** |
| `model/place-order-handler.js` | your application code |
| `check-domain.js` | whether the domain knows which broker this is |

1. **Find the four stages of the pump in `message-pump.js`.** Get is labelled. Where are the
   other three?
2. **`PlaceOrderHandler.handle` takes a `GetMessage`.** What is a `GetMessage`, which
   package does it come from, and what is it doing in a method about placing orders?
3. **`model/place-order-handler.js` names `amqplib`.** `model/` is the domain.
   Why does the domain need the broker's client library? Should it? (`node check-domain.js`
   answers the first half mechanically. The second half is yours.)
...
```
node sender.js  # a perfectly good WIDGET-1
```
...
`PlaceOrder` — and call it from the pump, between getting the message and dispatching it.
  ...
  - **Where it goes:** the contract belongs in `simple-messaging/` with the rest of the seam;
...
`handle(delivery)` becomes `handle(order)`. When you are done:

- **`node check-domain.js` passes.** It walks `model/` and fails if anything under it names
  the broker's library — in an `import`, in a `require`, or in the `import('amqplib')` a JSDoc
  comment uses to borrow a type. In the C# version of this exercise that check is a compile
  error; JavaScript has no compiler to do it for you, so the script does it instead. Either
  way it is a check rather than a matter of opinion — and right now it fails.
- Nothing in `model/` mentions `amqplib` at all.
...
**And when all three are done, delete the banner at the top of `simple-messaging/message-pump.js`.** It says the
...
- Send `node sender.js burst 50` and watch the queue drain. Then start a
