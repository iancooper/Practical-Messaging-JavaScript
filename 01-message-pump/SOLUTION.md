...
### 1. The handler took a `GetMessage` ###
...
- **`model/` had to name `amqplib` to say what the handler takes.** The domain now has a
  dependency on a specific broker, and it is the quietest kind: it lives in a JSDoc comment,
  so nothing at run time will ever complain about it. Swap broker and every file under
  `model/` needs revisiting, and nothing will stop you shipping it — which is why
  `check-domain.js` exists.
- **The handler cannot be called by anything else.** Not a unit test, not an HTTP endpoint, not
  ...
  `GetMessage`, which means fabricating a delivery tag and a byte array.
- **The pump cannot tell your failure from its own.** A `TypeError` and an
  `UnknownSkuError` both come out of the same call, and the pump has no way to know which
...
calls it. The handler takes `PlaceOrder` and returns. Delete the `amqplib` reference from
`model/place-order-handler.js` and let `node check-domain.js` find anything you missed.
...
- **It must fail in its own way.** When the body will not deserialize, the mapper catches
  whatever the serializer threw and raises `UnmappableMessageError` instead. That is not
  ...
  *I read it and the work failed*, and exercise 2's entire fix is that distinction. A mapper
  that lets `TypeError` escape has done the translation and not the classification.
- **It lives on the domain side of the seam.** The contract goes in `simple-messaging/` with the
...
```
    ...
    consumer.acknowledge(delivery);                    // <-- here
...
unwound straight out of `run`, out of `receiver.js`, and took the process with it. One malformed body
...
- If you `reject(delivery, true)` an unmappable body, you have built an infinite loop. It will
  never map. It will come straight back. Your log will fill up at the speed of the network.
- If you `reject(delivery, false)` it, the broker deletes it — the message is still gone, you
...
The gateway owns the broker; nothing outside `simple-messaging` names a RabbitMQ type. The pump
